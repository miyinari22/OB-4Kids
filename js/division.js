function generateQuestion() {
    const divisor = Math.floor(Math.random() * 9) + 1; // Número aleatorio entre 1 y 9
    const quotient = Math.floor(Math.random() * 10) + 1; // Número aleatorio entre 1 y 10
    const dividend = divisor * quotient; // Asegurar divisiones exactas
    ...
    let attempts = 0; // Limitar intentos de generación

    while (options.size < 4 && attempts < 20) {
        const wrongAnswer = Math.floor(Math.random() * 10) + 1; // Opciones aleatorias entre 1 y 10
        // Asegurarse de que las respuestas sean diferentes
        if (!options.has(wrongAnswer) && wrongAnswer !== correctAnswer) {
            options.add(wrongAnswer);
        }
        attempts++;
    }

    // Si no se pudieron generar suficientes opciones en los intentos, agregar números por defecto
    while (options.size < 4) {
        options.add((correctAnswer + options.size) % 10 + 1); // Números consecutivos
    }
